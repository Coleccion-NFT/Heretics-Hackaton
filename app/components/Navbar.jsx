            ...
            <div className="flex flex-col h-5/6 justify-evenly">
                {NavItems.navitems.map((navitem) => (
                    <div className="block w-full" key={navitem.id}>
                        ...
                        <Link
                            href={`/${navitem.path}`}
                            className="hidden group md:flex sm:flex-row relative focus:border-black text-xs tracking-wider hover:border-amber-500 text-black focus:font-semibold focus:bg-black focus:text-white border-2 border-white border-solid pl-10 pr-3 py-3 rounded-xl"
                        >
                            <span className="hidden lg:block absolute inset-y-0 -left-8 w-4 h-auto rounded-r-md group-focus:bg-black"></span>
